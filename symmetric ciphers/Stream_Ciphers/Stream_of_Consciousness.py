# Danh sách 22 ciphertexts bạn đã lấy được từ server
stream_hex = [
    ...
    'b8c5648ecff80dd8f4b90dec5ea95113132b6d4595e6de5d55f4f974388e04c0800c6edc11e918582970c14f71', 
...
ciphertexts = [bytes.fromhex(ct) for ct in stream_hex]
max_len = max(len(ct) for ct in ciphertexts)

# Khởi tạo keystream và mảng lưu trạng thái keystream đã tìm được
keystream = [0] * max_len
known_key = [False] * max_len

# 1. TỰ ĐỘNG GIẢI MÃ: Kỹ thuật Space Heuristic
for col in range(max_len):
    for i, c1 in enumerate(ciphertexts):
        ...
        # Thử XOR c1 với tất cả các ciphertext còn lại tại cột 'col'
        for j, c2 in enumerate(ciphertexts):
            if i == j or col >= len(c2): continue
            valid_chars += 1
            
            xor_val = c1[col] ^ c2[col]
            # Ký tự chữ cái (a-z, A-Z) -> Dấu hiệu của việc XOR với khoảng trắng
            if chr(xor_val).isalpha():
                space_count += 1
        
        # Nếu tỷ lệ > 70% là chữ cái, ta tự tin đoán c1[col] là dấu cách (' ')
        if valid_chars > 0 and (space_count / valid_chars) > 0.7:
            keystream[col] = c1[col] ^ ord(' ')
            known_key[col] = True
            break

# 2. CRIB DRAGGING THỦ CÔNG: Điền nốt các chỗ trống (*)
# Dựa vào những câu bạn đã mò ra được (VD: "I'm unhappy", "crypto{")
def patch_keystream(ct_idx, known_text, offset=0):
    for i, char in enumerate(known_text):
        if offset + i < len(ciphertexts[ct_idx]):
            keystream[offset + i] = ciphertexts[ct_idx][offset + i] ^ ord(char)
            known_key[offset + i] = True

# Theo danh sách các pt bạn tìm được:
# Patch flag format vào CT đầu tiên bạn thấy khớp (bạn có thể đổi index nếu sai)
# Nhìn vào kết quả của Auto, bạn sẽ thấy dòng nào chứa flag để patch tiếp
patch_keystream(4, "crypto{") 
...
for idx, ct in enumerate(ciphertexts):
    ...
    for i in range(len(ct)):
        if known_key[i]:
            # Đảm bảo ký tự in ra được (printable), nếu rác thì in dấu ?
            char = chr(ct[i] ^ keystream[i])
            pt += char if char.isprintable() else '?'
        else:
            pt += "*" # Những ký tự chưa đoán được
    
    # Highlight dòng chứa chữ crypto
    if "crypto{" in pt:
        print(f"\n[!] FLAG FOUND IN ROW {idx}:")
        print(f"\033[92m[{idx:02d}] {pt}\033[0m\n")
    else:
        print(f"[{idx:02d}] {pt}")